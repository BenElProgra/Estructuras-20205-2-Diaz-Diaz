...
Cola buscarClave(HashTable* hashtable,void *dato)
{
	...
	*/
    ...
    for(int k = 0; k < hashtable->tam; k++)
    {
        int nuevo_indice = rehash(indice, k, hashtable->tam);
...
//Forma chafa porque no aplica principios de hash.
//La fomra correcta seria un for de un limite al otro, hacer el folding, hash y rehash metiendo todos los datos.
Cola buscarClave_Rango(HashTable *hashtable, void *datoInf, void *datoSup)
{
	Cola cola = inicializarCola();
	if(!validarHashTable(hashtable))
		return cola;
	...
	for(int i = 0; i < hashtable->tam; i++)
	{
		...
		{
			void *dato = hashtable->tabla[i];

			// Dato es mayor o igual al LimiteInferior?
			int cmpInf = hashtable->comparar(dato, datoInf);
			// Dato es menor o igual al LimiteSuperior?
			int cmpSup = hashtable->comparar(dato, datoSup);

			//Si Cumple con los dos esta entre el rango y se agrega
...
//Limpiar Hash
void vaciarHash(HashTable *hashtable){
	for(int i = 0; i<hashtable->tam;i++)
