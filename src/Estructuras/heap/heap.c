...
void imprimirHeapIndice(Heap heap)
{
...
void eliminarPosicionHeap(Heap *heap,int posicion)
{
	if(heap->cantidad == 0)
		return;
	heap->cantidad--;
	heap->arr[posicion] = heap->arr[heap->cantidad];
	//DECREMENTAR MEMORIA	
	heap->arr = realloc(heap->arr,sizeof(NodoA*)*heap->cantidad);
	heapify(heap);	
}
